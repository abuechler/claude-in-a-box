...
docker build --platform linux/amd64 --build-arg VERSION=1.0.2-amd64 -t claude-in-a-box:flutter-1.0.2 -f docker/Dockerfile.flutter docker/
```
...
2. **Variant Layers**:
    ...
    - `docker/Dockerfile.flutter`: Adds Flutter SDK (forces linux/amd64 on ARM hosts)
